...
func enemyInRange(_ grid: [[Unit]], x: Int, y: Int, enemy: Character) -> (Int, Int)? {
...
func findNearestEnemy(_ grid: [[Unit]], x: Int, y: Int) -> (Int, Int) {
    var queue = [(x, y)];
...
func cloneGrid(_ start: [[Unit]]) -> [[Unit]] {
    var retval: [[Unit]] = [];
    for line in start {
        var retline: [Unit] = [];
        for unit in line {
...
func runCombat(_ start: [[Unit]], elfPower: Int) -> Int? {
    var grid = cloneGrid(start);
    var turn = 0;
    outer: while true {
        turn += 1;
        for x in 0..<grid.count {
            for y in 0..<grid[0].count {
                if (grid[x][y].kind == "E" || grid[x][y].kind == "G") && grid[x][y].last_move < turn {
                    ...
                    var nx = x;
                    var ny = y;
                    if enemyInRange(grid, x: x, y: y, enemy: grid[x][y].enemy()) == nil {
                        (nx, ny) = findNearestEnemy(grid, x: x, y: y);
                        if nx == Int.max {
                            break outer;
                        }
                        if ny != Int.max {
                    ...
                    if let (ex, ey) = enemyInRange(grid, x: nx, y: ny, enemy: grid[nx][ny].enemy()) {
                        if grid[nx][ny].kind == "E" {
                            grid[ex][ey].hp -= elfPower;
                        } else {
                            grid[ex][ey].hp -= 3;
                        }
                        if grid[ex][ey].hp <= 0 {
                            if grid[ex][ey].kind == "E" {
                                return nil;
                            } else {
                                grid[ex][ey].kind = ".";
                            }
...
while true {
    epower += 1;
    if let result = runCombat(grid, elfPower: epower) {
        print(result);
        break;
    }
}
