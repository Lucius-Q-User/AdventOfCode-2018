...
fn unitstack_attack_order<'a>(a: &UnitStack<'a>, b: &UnitStack<'a>) -> Ordering {
...
fn main() {
    ...
    while !inf.is_empty() && !imms.is_empty() {
        ...
        for stack in &mut inf {
            if let Some(target) = imms.iter().filter(|x| !taken_target.contains(&x.id)).max_by(|a, b| {
        ...
        for stack in &mut imms {
            if let Some(target) = inf.iter().filter(|x| !taken_target.contains(&x.id)).max_by(|a, b| {
                let o = damage_to_stack(stack, a).cmp(&damage_to_stack(stack, b));
                let o = o.then(eff_power(a).cmp(&eff_power(b)));
                o.then(a.init.cmp(&b.init))
            }) {
    ...
    println!("{:?}", max(imms.iter().map(|x| x.units.get()).fold(0, |a, b| a + b), inf.iter().map(|x| x.units.get()).fold(0, |a, b| a + b)));
}
